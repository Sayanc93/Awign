class Api::V1::BaseController < ApplicationController
  ...
  def render_errors(error_message, status = 422)
  ...
  def authenticate_user_using_auth_token
    ...
    @user = user_email && find_user(user_email)
  ...
  def find_user(user_email = "")
    find_project_manager(user_email) || find_intern(user_email)
  end

  def find_project_manager(user_email = "")
    ProjectManager.find_by("lower(email) = ? ", user_email.downcase)
  end

  def find_intern(user_email = "")
    Intern.find_by("lower(email) = ? ", user_email.downcase)
  end
end
